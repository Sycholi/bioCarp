...
- 临床研究设计、单臂和双臂研究、样本量计算、目标试验模拟、真实世界研究、EHR、REDCap、OMOP、FHIR、CDISC 和数据质量检查
- 系统综述、meta 分析、网状 meta、MR、药物警戒、GWAS、PRS、fine mapping、colocalization、QTL、TWAS 和 biobank 分析
- 10x、BD、MGI、BGI、Singleron/新格元、Parse、ScaleBio、Fluent、SMART-seq、空间平台、质谱平台和影像平台的差异检查
...
```text
使用 biocarp 评估这个临床队列能否做双臂研究样本量估计、目标试验模拟和生存结局分析。
```

```text
使用 biocarp 为这个蛋白质组和代谢组项目设计分析流程，并说明不同质谱平台和参数设置对结果的影响。
```

## 核心特性

- 先确认研究问题、数据结构、平台、参数和终点，再执行分析
...
`SKILL.md` 保存核心规则。`references/` 保存按任务调用的流程、工具、平台、参数、文献和问题记录。`scripts/` 保存文献检索和代码风格摘要等辅助脚本。
